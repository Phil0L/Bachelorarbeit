...
import type { ImportDoneEvent, ImportXMLResult, SaveXMLResult } from 'bpmn-js';
...
export class DiagramComponent implements AfterContentInit, OnChanges, OnDestroy, OnInit {
  ...
  loadUrl(url: string): Subscription {
    return (
      this.http.get(url, { responseType: 'text' }).pipe(
        ...
        catchError(err => {
          // If an error occurs when trying to load the diagram, create an empty diagram
  ...
  createEmptyDiagram(): void {
    // Initialize a new diagram object
    // The specifics of this will depend on the library you're using to manage diagrams
    this.bpmnJS.createDiagram();
  }

  saveDiagram(): Promise<SaveXMLResult> {
    return this.bpmnJS.saveXML();
  }
