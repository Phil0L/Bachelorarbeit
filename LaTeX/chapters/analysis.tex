...
Gleichzeigt wird zudem auch zwischen den verwendeten Formaten XML und JSON, welche auf 
...
Um auch zu testen, ob die Modelle in der Lage sind die gewünschte Ausgabesprache zu erkennen und zu verwenden,
...
ein \texttt{kritischer Fehler}. Tritt ein kritischer Fehler auf, wird die Generierung wiederholt, 
bis keine kritischen Fehler erstellt werden.
Ein Fehler, durch den ein einzelnes Element,
nicht angezeigt werden kann, ist ein \texttt{elementarer Fehler}.
...
Die erzeugten Diagramme enthalten keine formalen Fehler
und halten die grundlegenden BPMN-2.0-Konventionen ein. 
Damit zeigen diese Modelle eine hohe Eignung für die Modellierung komplexer Prozesse, 
sofern JSON als Zielformat verwendet wird. 
...
unnötige Ergänzungen, insbesondere im XML-Format. 

Abgesehen von den ausgearbeiteten Komponentenfehler, lassen sich noch einige weitere Eigenheiten erkennen.
Bei Claude Sonnet 4.5 und Gemini 2.5 Flash JSON wurden alle Message Flows fehlerhaft erzeugt, wodurch sie nicht vorhanden sind.
...
Es gab ausserdem auch Probleme die Elemente in die jeweiligen Lanes zu platzieren bei ChatGPT 5.2 mit XML sowie bei ChatGPT 5.1 mit JSON.
...
Erstellung semantisch korrekter BPMN-Diagramme aus komplexen Textbeschreibungen nur eingeschränkt geeignet.
...
soll nun noch die Frage beantwortet werden.
...
Wie bereits bei den komplexeren Prozessbeschreibungen wird nun die formale Richtigkeit untersucht.
\footnote{Wie die formale Richtigkeit festgestellt wird, wir auf Seite~\ref{sec:correctness-formal} gezeigt.}
...
Dies bedeutet, dass alle erzeugten BPMN-Diagramme grundsätzlich darstellbar waren und keine 
strukturellen Fehler enthielten.
...
Eine geringe Menge an elementaren Fehlern sind bei Gemini 2.5 Flash, ChatGPT 4.1 und Grok 4.1 Fast, jeweils 
...
auf fehlende und unnötige Elemente getestet werden.
\footnote{Wie die semantische Richtigkeit festgestellt wird, wir auf Seite~\ref{sec:correctness-semantic} gezeigt.}
...
Alleinig Gemini 2.5 Pro erziehlt im Vergleich zu den anderen Modellen sehr gute Diagramme, die
kaum Nachbearbeitung benötigen und zusätzlich noch optisch sehr ansprechend sind.
...
Aus den Daten ind Abbildung~\ref{fig:timing-streaming} geht hervor, dass bei jedem Modell die Variante des Streamings die Variante 
...
Als nächstes soll nun untersucht werden welches der zwei implementierten Formate \texttt{JSON}
...
Streamstart im Vergleich zu der Diagrammgenereiung sowie der API Antwort sehr wenig Zeit beanspruchen,
Sind die exakten Daten auch noch in Tabelle~\ref{tab:timing-xmljson} zu sehen.
...
Hierfür wird Prompt~\ref{prompt:speed-elements} verwendet.
...
Insgesamt wird dadurch klar, dass JSON für zeitperformanzkritische Anwendungsfälle, 
...
implementierten KI-Modellanbieter, im Stand von November 2025.
