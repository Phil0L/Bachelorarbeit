...
die verwendet wird, um Geschäftsprozesse so zu dokumentieren, dass sie leicht verständlich sind. 
Ähnlich wie ein Architekt Baupläne nutzt, um Gebäude darzustellen, verwenden Business-Analysten 
BPMN 2.0, um visuelle Darstellungen organisatorischer Abläufe zu erstellen.

BPMN 2.0 Diagramme~\cite{omg2014bpmn2} zeigen die Abfolge von Tätigkeiten von Anfang bis Ende, 
einschließlich was passiert, wann es passiert und wer jede Aufgabe ausführt. 
Die Hauptkomponenten von BPMN 2.0 sind:
...
Ein vollständiges BPMN 2.0 Diagramm sieht beispielsweise so aus wie in Abbildung~\ref{fig:bpmn-elements}.
...
Sie basieren auf Deep-neural Networks, insbesondere auf Transformer-Architekturen, die große 
...
wandelt diese in BPMN-Elemente wie Tasks, Events, Gateways, Pools und Lanes um. 
LLMs können dabei sowohl die logische Abfolge der Prozessschritte erkennen als auch Verzweigungen 
sowie Kommunikationsflüsse zwischen Beteiligten identifizieren.
...
Auf diese Weise können LLMs dabei helfen, die Erstellung von Prozessmodellen zu beschleunigen 
und auch komplexe Abläufe automatisch darstellen.

\section{Chain of Thought}

Chain of Thought (CoT) beschreibt eine Methode, bei der ein Large Language Model (LLM) seine 
...
CoT ermöglicht es LLMs wie ChatGPT, Prozessbeschreibungen in einzelne Handlungsschritte zu zerlegen, diese 
...
auftreten und wie die Kommunikation zwischen Akteuren oder Abteilungen abgebildet werden muss. 
...
Empfänger, wobei die Daten nicht als Ganzes sondern in Teilen
übertragen werden. 
...
durch eine Anzeige des aktuellen Zustands in Echtzeit.
...
Die Textfragmente werden hier in der Regel auch Delta genannt.
Auf diese Weise lässt sich die Nutzererfahrung deutlich verbessern, da Rückmeldungen sofort sichtbar 
sind und Wartezeiten minimiert werden. 

\section{Base64}

Base64 ist ein Kodierungsverfahren, das Binärdaten wie zum Beipsiel Bilder in eine reine Textdarstellung überführt, 
...
Zur Kodierung werden die Zeichen A-Z, a-z, 0-9, + und / verwendet sowie ein = am Ende.
...
Data URLs~\cite{rfc2397} übertragen. 
...
\begin{enumerate}
    ...
    \item dem eigentlichen Datenbereich mit einem = am Ende
\end{enumerate}
...
Zunächst erstellt das Modell einen ersten Entwurf.
...
BPMNGen ist ein Projekt der Universität Ulm, Institut für Datenbanken und Informationssystem.
...
Das Frontend ermöglicht es den Nutzerinnen und Nutzern, mit dem BPMNGen System zu interagieren.
...
Alle generierten Diagramme werden in einer durch Prisma verwalteten Postgres Datenbank gespeichert.
